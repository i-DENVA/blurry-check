...
export class PDFAnalyzer {
  ...
  private calculateRenderedPageMetrics(
    ...
    pageNumber: number,
    rotation: number,
  ): NonNullable<BlurAnalysisResult['metrics']['pdfPageMetrics']> {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (luminance < 245) {
          contentSum += luminance;
          contentSumSquares += luminance * luminance;
          contentCount++;
    ...
    const contentVariance =
      contentCount > 0
        ? contentSumSquares / contentCount - contentBrightness * contentBrightness
        : variance;
    ...
    const edgesTouchingBoundary = Object.entries(marginRatios)
      .filter(([, ratio]) => ratio < 0.015)
      .map(([edge]) => edge);
    ...
    return {
      ...
      documentFrame: {
        detected,
        marginRatios,
        edgesTouchingBoundary,
        perspectiveScore,
        isLikelyCropped: edgesTouchingBoundary.length > 0,
        hasPerspectiveDistortion: perspectiveScore < 65,
      },
    };
  }
  ...
  private estimatePerspectiveScore(imageData: ImageData): number {
    ...
    const avgWidth = (items: Array<{ left: number; right: number }>) =>
      items.reduce((s, i) => s + i.right - i.left, 0) / Math.max(items.length, 1);
    const topBottomRatio =
      Math.min(avgWidth(topBand), avgWidth(bottomBand)) /
      Math.max(avgWidth(topBand), avgWidth(bottomBand), 1);
    const avgLeft = (items: Array<{ left: number; right: number }>) =>
      items.reduce((s, i) => s + i.left, 0) / Math.max(items.length, 1);
  ...
  private log(message: string, ...args: any[]) {
    if (this.config.debug) console.log(`[PDFAnalyzer] ${message}`, ...args);
  }
  ...
  private async checkPdfPageQuality(
    pdf: any,
    pageNumber: number,
    maxRenderScale = 2.0,
    ...
    for (const scale of scales) {
      ...
      const pdfPageMetrics = this.calculateRenderedPageMetrics(
        imageData,
        pageNumber,
        viewport.rotation ?? page.rotate ?? 0,
      );

      const pdfBlurDetector = new BlurDetector({
        ...this.config,
  ...
  private analyzePageContent(textContent: any, pageNumber: number) {
    ...
    const hasHeaderKeywords = /bill|statement|invoice|report|summary|account|period/i.test(
      totalText,
    );
    ...
    this.log(
      `Page ${pageNumber} content analysis: textLength=${textLength}, textDensity=${textDensity.toFixed(1)}, isLikelyHeader=${isLikelyHeaderPage}, isCertificate=${isCertificateDocument}`,
    );
    return { isLikelyHeaderPage, textDensity, hasLowTextContent, isCertificateDocument };
  }

  private async analyzeTextSharpness(
    pdf: any,
    pageNumber: number,
    maxRenderScale = 3.0,
  ): Promise<{ textSharpnessScore: number; isTextBlurry: boolean; textMetrics: any }> {
    const page = await pdf.getPage(pageNumber);
    ...
    const imageData = context.getImageData(0, 0, canvas.width, canvas.height);
    const result = this.calculateTextSharpness(imageData);
    this.log(`Page ${pageNumber} text sharpness analysis:`, result);
    return result;
  }

  private calculateTextSharpness(imageData: ImageData): {
        ...
        for (let dy = 0; dy < sampleSize; dy++) {
          for (let dx = 0; dx < sampleSize; dx++) {
            ...
            if (dx < sampleSize - 1 && dy < sampleSize - 1) {
              ...
              const gx =
                0.299 * data[rIdx] + 0.587 * data[rIdx + 1] + 0.114 * data[rIdx + 2] - gray;
              const gy =
                0.299 * data[dIdx] + 0.587 * data[dIdx + 1] + 0.114 * data[dIdx + 2] - gray;
    ...
    return {
      ...
      isTextBlurry,
      textMetrics: { avgVariance, avgEdgeIntensity, sampleCount, threshold: 0.8 },
    };
  }
  ...
  async analyzePDF(
    ...
    try {
      ...
      if (Array.isArray(samplePages))
        pagesToAnalyze = samplePages.filter((p) => p >= 1 && p <= totalPages).slice(0, maxPages);
      else if (samplePages === 'first') pagesToAnalyze = [1];
      else if (samplePages === 'smart') {
        const smart: number[] = [];
        for (let i = 1; i <= totalPages; i++)
          if (i === 1 || i === totalPages || i % 5 === 0) smart.push(i);
        pagesToAnalyze = smart.slice(0, maxPages);
      } else
        pagesToAnalyze = Array.from({ length: totalPages }, (_, i) => i + 1).slice(0, maxPages);

      if (pagesToAnalyze.length < totalPages && maxPages < totalPages) {
        incomplete = true;
        incompleteReason = `Page cap: analyzed ${pagesToAnalyze.length}/${totalPages}`;
      }
      const skippedPages = Array.from({ length: totalPages }, (_, i) => i + 1).filter(
        (p) => !pagesToAnalyze.includes(p),
      );
      ...
      for (const i of pagesToAnalyze) {
        if (Date.now() - startTime > timeoutMs) {
          incomplete = true;
          incompleteReason = `Timeout: stopped at page ${i}`;
          break;
        }
        ...
        try {
          ...
          if (textContent.items.length > 0) {
            try {
              ...
              if (contentAnalysis.isCertificateDocument) {
                finalIsBlurry = textSharpness.textSharpnessScore < 0.3;
                this.log(`Page ${i} identified as certificate - lenient blur`);
              } else if (contentAnalysis.isLikelyHeaderPage) {
                finalIsBlurry = textSharpness.textSharpnessScore < 0.5;
                this.log(`Page ${i} identified as header/logo - lenient blur`);
              }
              ...
              pageResults.push({
                ...
                metrics: { ...pageAnalysis.metrics, textSharpness, contentAnalysis },
              });
          ...
          corruptedPages.push({
            page: i,
            error: error instanceof Error ? error.message : 'Unknown page analysis error',
          });
        }
      }
      ...
      this.log(
        `PDF analysis complete. Scanned: ${finalIsScanned}, Text length: ${extractedText.length}`,
      );
      ...
      if (pageResults.length > 0) {
        ...
        if (pageResults.length > 1) {
          ...
          if (pageResults[0].isBlurry && blurryNonFirst.length === 0) {
            isQualityGood = true;
            this.log('First page blurry but rest clear - likely false positive');
      ...
      const result: PDFAnalysisResult = {
        ...
        corruptedPages: corruptedPages.length ? corruptedPages : undefined,
        incomplete,
        incompleteReason,
        totalPages,
        skippedPages: skippedPages.length ? skippedPages : undefined,
      };
  ...
  async isGoodQuality(file: File): Promise<boolean> {
    return (await this.analyzePDF(file)).isQualityGood;
  }
}
