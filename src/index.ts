...
import type {
  BlurDetectionConfig, BlurAnalysisResult, PDFAnalysisResult,
  FileAnalysisOptions, ImageInput, QualityValidationResult,
  UploadValidationOptions, PDFPerformanceOptions,
} from './types';
...
export class BlurryCheck {
  ...
  constructor(config: BlurDetectionConfig = {}) {
    this.config = { edgeWidthThreshold: 0.5, laplacianThreshold: 100, method: 'edge', debug: false, ...config };
    this.blurDetector = new BlurDetector(this.config);
    this.pdfAnalyzer = new PDFAnalyzer(this.config);
  }

  async isImageBlurry(input: ImageInput) { return this.blurDetector.isBlurry(input); }
  async analyzeImage(input: ImageInput) { return this.blurDetector.analyzeImage(input); }
  async isPDFGoodQuality(file: File) { return this.pdfAnalyzer.isGoodQuality(file); }
  async analyzePDF(file: File, perfOptions?: PDFPerformanceOptions) { return this.pdfAnalyzer.analyzePDF(file, perfOptions); }
  ...
  async analyzeFile(file: File, options: FileAnalysisOptions = {}) {
    ...
    if (['png', 'jpg', 'jpeg', 'gif', 'bmp', 'webp'].includes(ext)) return new BlurDetector(mergedConfig).analyzeImage(file);
  ...
  async validateUpload(file: File, options: UploadValidationOptions = {}) {
    const ext = extensionFor(file);
    if (file.type === 'application/pdf' || ext === 'pdf') return validatePDFQuality(file, { ...options }, this.config);
  ...
  getConfig(): BlurDetectionConfig { return { ...this.config }; }
}

export async function isImageBlurry(input: ImageInput, config?: BlurDetectionConfig) { return new BlurryCheck(config).isImageBlurry(input); }
export async function isPDFGoodQuality(file: File, config?: BlurDetectionConfig) { return new BlurryCheck(config).isPDFGoodQuality(file); }
export async function analyzeFile(file: File, options?: FileAnalysisOptions) { return new BlurryCheck(options).analyzeFile(file, options); }
export async function validateImage(input: ImageInput, options?: UploadValidationOptions) { return new BlurryCheck(options).validateImage(input, options); }
export async function validateUpload(file: File, options?: UploadValidationOptions) { return new BlurryCheck(options).validateUpload(file, options); }
