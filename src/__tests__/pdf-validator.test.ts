...
function page(o: {
  ...
  const w = o.w ?? 1200;
  const h = o.h ?? 1600;
  const ps = o.perspectiveScore ?? 85;
  const edges = o.edges ?? [];
  return {
    isBlurry: o.isBlurry ?? false,
    confidence: 0.9,
    method: 'edge',
    metrics: {
      ...
      pdfPageMetrics: {
        ...
        brightness: o.brightness ?? 180,
        contrast: o.contrast ?? 55,
        minLuminance: o.minL ?? 20,
        maxLuminance: o.maxL ?? 245,
        nonWhiteRatio: o.nonWhite ?? 0.15,
        contentBrightness: o.cBrightness ?? 120,
        contentContrast: o.cContrast ?? 45,
        glarePixelRatio: o.glare ?? 0.01,
        documentFrame: {
          detected: true,
...
function pdf(p: {
  ...
  analyzed?: number;
  textLen?: number;
  pages?: BlurAnalysisResult[];
  corrupt?: Array<{ page: number; error: string }>;
}): () => Promise<PDFAnalysisResult> {
  return async () => ({
    isQualityGood: p.good ?? true,
    isScanned: p.scanned ?? false,
    pagesAnalyzed: p.analyzed ?? 1,
    textLength: p.textLen ?? 500,
    pageResults: p.pages ?? [],
    corruptedPages: p.corrupt,
  });
}
...
describe('validatePDFQuality', () => {
  ...
  it('passes digital-clear.pdf — low_text_density and cover_page are non-blocking', async () => {
    (globalThis as any).__mockAnalyzePDF = pdf({
      ...
      textLen: 5000,
      pages: [
        page({ isBlurry: false, brightness: 200, contrast: 60, lowText: true, headerPage: true }),
      ],
    });
    const r = await validatePDFQuality(file, { mode: 'document' });
  ...
  it('fails low-contrast.pdf for low_contrast (not text-density)', async () => {
    (globalThis as any).__mockAnalyzePDF = pdf({
      pages: [page({ contrast: 10, minL: 30, maxL: 220, cContrast: 10 })],
    });
  ...
  it('fails low_text_density in ocr mode', async () => {
    ...
    expect(r.ok).toBe(false);
    expect(r.issues).toContain('low_text_density');
  });
  ...
  it('passes rotated-page.pdf when expectedOrientation is not set', async () => {
    (globalThis as any).__mockAnalyzePDF = pdf({
      pages: [page({ orientation: 'landscape', rotation: 90 })],
    });
    const r = await validatePDFQuality(file, { mode: 'document' });
    expect(r.ok).toBe(true);
    expect(r.issues).not.toContain('rotated');
  });
  ...
  it('fails rotated-page.pdf with expectedOrientation: portrait', async () => {
    ...
    expect(r.ok).toBe(false);
    expect(r.issues).toContain('rotated');
  });
  ...
  it('fails perspective-document.pdf for perspective_distortion', async () => {
    ...
    const r = await validatePDFQuality(file, { mode: 'document' });
    expect(r.ok).toBe(false);
    expect(r.issues).toContain('perspective_distortion');
  });
  ...
  it('fails cropped-document.pdf for cropped', async () => {
    (globalThis as any).__mockAnalyzePDF = pdf({
      pages: [
        page({
          edges: ['top', 'right'],
          margins: { top: 0.005, right: 0.005, bottom: 0.1, left: 0.1 },
        }),
      ],
    });
    const r = await validatePDFQuality(file, { mode: 'document' });
    expect(r.ok).toBe(false);
    expect(r.issues).toContain('cropped');
  });
    ...
    (globalThis as any).__mockAnalyzePDF = pdf({
      pages: [
        page({
          ...
          maxL: 80,
          edges: ['bottom'],
          margins: { top: 0.1, right: 0.1, bottom: 0.005, left: 0.1 },
        }),
      ],
    });
    const r = await validatePDFQuality(file, { mode: 'document' });
  ...
  it('passes clear white document without false glare', async () => {
    ...
    expect(r.issues).not.toContain('too_bright');
  });
  ...
  it('returns corrupted_pdf for unreadable PDF', async () => {
    (globalThis as any).__mockAnalyzePDF = async () => {
      throw new Error('Failed to read PDF');
    };
    const r = await validatePDFQuality(file, { mode: 'document' });
    expect(r.ok).toBe(false);
    expect(r.issues).toContain('corrupted_pdf');
  });
  ...
  it('passes spoofed-pdf.pdf as valid (text signals non-blocking in document mode)', async () => {
    (globalThis as any).__mockAnalyzePDF = pdf({
      scanned: false,
      textLen: 5,
      pages: [page({ isBlurry: false, lowText: true })],
    });
    const r = await validatePDFQuality(file, { mode: 'document' });
    expect(r.ok).toBe(true);
    expect(r.warnings).toContain('low_text_density');
