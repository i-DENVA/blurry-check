...
const STRICTNESS_MULTIPLIERS: Record<
  StrictnessLevel,
  { scoreBump: number; thresholdMultiplier: number; sizeMultiplier: number }
> = {
...
export function resolveMode(
  ...
  return {
    ...base,
    minScore: Math.max(0, Math.min(100, base.minScore + mod.scoreBump)),
    edgeWidthThreshold: Math.max(
      0.05,
      Math.min(2, base.edgeWidthThreshold * mod.thresholdMultiplier),
    ),
    maxSizeMB: Math.round(base.maxSizeMB * mod.sizeMultiplier),
  };
}
