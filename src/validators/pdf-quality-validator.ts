...
export async function validatePDFQuality(
  ...
  const fileSizeCheck = makeCheck(
    sizeOk,
    sizeOk ? 100 : 45,
    sizeOk ? 'PDF size is acceptable.' : 'PDF too large.',
    { size: file.size, maxSizeBytes: maxBytes },
  );

  if (!fileCheck.ok) {
    const fi =
      fileCheck.details && typeof fileCheck.details === 'object' && 'issues' in fileCheck.details
    ...
    return {
      ...
      ok: false,
      status: 'fail',
      score: Math.min(fileCheck.score, fileSizeCheck.score),
      message: summaryFor(fi),
      type: 'pdf',
      checks: { file: fileCheck, fileSize: fileSizeCheck },
      issues: fi,
      warnings: [],
      recommendations: recommendationsFor(fi),
    };
  }

  let pdfAnalysis: PDFAnalysisResult;
  try {
    pdfAnalysis = await new PDFAnalyzer({
      ...config,
      ...opts,
      edgeWidthThreshold: opts.edgeWidthThreshold,
      method: opts.method,
    }).analyzePDF(file, {
    ...
    return {
      ...
      status: 'fail',
      score: 0,
      message: summaryFor(['corrupted_pdf']),
      type: 'pdf',
      checks: {
        file: fileCheck,
        fileSize: fileSizeCheck,
        corruptedPages: makeCheck(false, 0, 'PDF could not be read.', {
          error: error instanceof Error ? error.message : 'Unknown',
        }),
      },
      issues: ['corrupted_pdf'],
      warnings: [],
      recommendations: recommendationsFor(['corrupted_pdf']),
  ...
  const blocking: IssueCode[] = [],
    nonBlocking: IssueCode[] = [...allWarnings];
  ...
  const scannedCheck = makeCheck(
    true,
    pdfAnalysis.isScanned ? 80 : 100,
  ...
  const corruptedCheck = makeCheck(
    corrupted.length === 0,
    corrupted.length === 0 ? 100 : 35,
    corrupted.length === 0
      ? 'All pages were analyzed.'
      : `${corrupted.length} page${corrupted.length === 1 ? '' : 's'} could not be analyzed.`,
    { corruptedPages: corrupted },
  );
  ...
  return {
    valid: ok,
    ok,
    status: statusFor(ok, overallScore),
    score: overallScore,
    message: summaryFor(uniqueBlocking),
    type: 'pdf',
    checks: {
      file: fileCheck,
      fileSize: fileSizeCheck,
      scanned: scannedCheck,
      corruptedPages: corruptedCheck,
      sharpness: makeCheck(
        pages.every((p) => !p.issues.includes('blurry')),
        pages.some((p) => p.issues.includes('blurry')) ? 50 : 100,
        pages.some((p) => p.issues.includes('blurry'))
          ? 'One or more pages are blurry.'
          : 'Page sharpness looks good.',
      ),
      brightness: makeCheck(
        pages.every(
          (p) =>
            !['too_dark', 'too_bright', 'glare'].some((c) => p.issues.includes(c as IssueCode)),
        ),
        pages.some((p) =>
          ['too_dark', 'too_bright', 'glare'].some((c) => p.issues.includes(c as IssueCode)),
        )
      ...
      contrast: makeCheck(
        pages.every((p) => !p.issues.includes('low_contrast')),
        pages.some((p) => p.issues.includes('low_contrast')) ? 60 : 100,
        pages.some((p) => p.issues.includes('low_contrast'))
          ? 'One or more pages have low contrast.'
          : 'Contrast looks good.',
      ),
      orientation: makeCheck(
        pages.every((p) => !p.issues.includes('rotated')),
        pages.some((p) => p.issues.includes('rotated')) ? 55 : 100,
        pages.some((p) => p.issues.includes('rotated'))
          ? 'One or more pages appear rotated.'
          : 'Page orientation looks correct.',
      ),
      mobileCapture: makeCheck(
        pages.every(
          (p) =>
            !['cropped', 'perspective_distortion'].some((c) => p.issues.includes(c as IssueCode)),
        ),
        pages.some((p) =>
          ['cropped', 'perspective_distortion'].some((c) => p.issues.includes(c as IssueCode)),
        )
      ...
      textDensity: makeCheck(
        pages.every((p) => !p.issues.includes('low_text_density')),
        pages.some((p) => p.issues.includes('low_text_density')) ? 65 : 100,
        pages.some((p) => p.issues.includes('low_text_density'))
    ...
    debugMetrics: {
      ...
      incomplete: pdfAnalysis.incomplete ?? false,
      incompleteReason: pdfAnalysis.incompleteReason,
      totalPages: pdfAnalysis.totalPages,
      skippedPages: pdfAnalysis.skippedPages,
    },
  };
}
