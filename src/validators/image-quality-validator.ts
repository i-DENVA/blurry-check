...
import { score, makeCheck } from '../utils';
import {
  ...
  BLANK_NON_WHITE_RATIO_MAX,
  BLANK_CONTRAST_MAX,
  BLANK_CONTENT_CONTRAST_MAX,
} from '../constants';
import type {
  BlurAnalysisResult,
  ImageInput,
...
const ALL_IMAGE_CHECKS: QualityCheckName[] = [
  'file',
  'format',
  'fileSize',
  'resolution',
...
function pixelMetrics(imageData: ImageData) {
  ...
  return {
    brightness: b,
    contrast: Math.sqrt(Math.max(v, 0)),
    minLuminance: minL,
    maxLuminance: maxL,
    contentBrightness: cCount ? cSum / cCount : b,
    contentContrast: cCount
      ? Math.sqrt(Math.max(cSumSq / cCount - (cSum / cCount) ** 2, 0))
      : Math.sqrt(Math.max(v, 0)),
    nonWhiteRatio: cCount / count,
    glarePixelRatio: gCount / count,
  };
}
...
function isBlankContent(m: ReturnType<typeof pixelMetrics>) {
  const nearZero =
    m.nonWhiteRatio <= BLANK_NON_WHITE_RATIO_MAX ||
    (m.contrast <= BLANK_CONTRAST_MAX && m.contentContrast <= BLANK_CONTENT_CONTRAST_MAX);
...
function brightnessCheck(
  m: ReturnType<typeof pixelMetrics>,
  issue?: IssueCode,
): QualityCheckResult {
  const ok = !issue;
  const s = !issue
  ...
  return makeCheck(
          ...
          : 'Image is too bright.',
    {
      ...
      glareLuminanceThreshold: GLARE_LUMINANCE_THRESHOLD,
      documentMaxLuminanceMin: DOCUMENT_MAX_LUMINANCE_MIN,
      documentContentBrightnessMin: DOCUMENT_CONTENT_BRIGHTNESS_MIN,
      documentContentMaxLuminanceMin: DOCUMENT_CONTENT_MAX_LUMINANCE_MIN,
    },
  );
}
...
function contrastCheck(m: ReturnType<typeof pixelMetrics>): QualityCheckResult {
  ...
  return makeCheck(
    ...
    (m.contrast / 55) * 100,
    ok
      ? hasReadable && m.contrast < CONTENT_AWARE_CONTRAST_THRESHOLD
      ...
      : 'Image has low contrast.',
    {
      ...
      contentBrightness: m.contentBrightness,
      contentContrast: m.contentContrast,
      readableContentMinContrast: CONTRAST_READABLE_CONTENT_MIN_CONTRAST,
      contrastReadableContentMinContrast: CONTRAST_READABLE_CONTENT_MIN_CONTRAST,
    },
  );
}
...
function checkResolution(
  ...
  if (w < minW || h < minH)
    return {
      ...
      details: { width: w, height: h, minWidth: minW, minHeight: minH },
    };
  if ((maxW && w > maxW) || (maxH && h > maxH))
    return {
      ok: false,
      msg: 'Image dimensions are larger than allowed.',
      details: { width: w, height: h, maxWidth: maxW, maxHeight: maxH },
    };
  return {
    ok: true,
...
function scoreBlur(
  ...
  if (typeof lv === 'number')
    s =
      typeof ep === 'number'
        ? Math.max(s, (lv / (laplaceThreshold * 2)) * 100)
        : (lv / (laplaceThreshold * 2)) * 100;
  if (blur.isBlurry) s = Math.min(s, 59);
  return makeCheck(
    !blur.isBlurry,
    s,
    blur.isBlurry ? 'Image appears blurry.' : 'Sharpness looks good.',
...
export async function validateImageQuality(
  ...
  if (input instanceof File && requested.some((n) => ['file', 'format', 'fileSize'].includes(n))) {
    ...
    if (requested.includes('format'))
      checks.format = {
        ...fc,
        details: { ...(fc.details as Record<string, unknown>), category: 'format' },
      };
    if (requested.includes('fileSize')) {
      ...
      checks.fileSize = makeCheck(
        ok,
        ok ? 100 : 50,
        ok ? 'File size is acceptable.' : 'File is too large.',
        { size: input.size, maxSizeBytes: maxBytes },
      );
    }
  }
  ...
  if (requested.includes('blur')) {
    ...
    const bc = scoreBlur(
      blurAnalysis,
      opts.edgeWidthThreshold ?? 0.3,
      opts.laplacianThreshold ?? 150,
    );
    checks.blur = bc;
  ...
  return {
    valid: ok,
    ok,
    status: ok ? 'pass' : overall >= minS ? 'warning' : 'fail',
    score: overall,
    message: summaryFor(uniqueIssues),
    type: 'image',
    checks,
    recommendations: recommendationsFor(uniqueIssues),
    issues: uniqueIssues,
    warnings: [],
    ...
    debugMetrics: {
      ...
      blank: {
        detected: blank,
        blankNonWhiteRatioMax: BLANK_NON_WHITE_RATIO_MAX,
        blankContrastMax: BLANK_CONTRAST_MAX,
        blankContentContrastMax: BLANK_CONTENT_CONTRAST_MAX,
      },
      resolvedThresholds: {
        brightness: {
          documentMaxLuminanceMin: DOCUMENT_MAX_LUMINANCE_MIN,
          documentContentBrightnessMin: DOCUMENT_CONTENT_BRIGHTNESS_MIN,
          documentContentMaxLuminanceMin: DOCUMENT_CONTENT_MAX_LUMINANCE_MIN,
          brightnessReadableContentMinContrast: BRIGHTNESS_READABLE_CONTENT_MIN_CONTRAST,
          glarePixelRatioThreshold: GLARE_PIXEL_RATIO_THRESHOLD,
          glareLuminanceThreshold: GLARE_LUMINANCE_THRESHOLD,
        },
        contrast: {
