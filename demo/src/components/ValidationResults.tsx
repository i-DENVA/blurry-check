...
import { CheckCircle, AlertTriangle, XCircle, ChevronDown, ChevronRight, Code2 } from 'lucide-react'
...
export default function ValidationResults({ result, config }: Props) {
  ...
  const checks = Object.entries(result.checks).filter(([_, v]) => v) as [string, QualityCheckResult][]
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className={`font-extrabold text-sm uppercase tracking-wider ${
                result.ok ? 'text-[var(--color-success-fg)]' : result.status === 'warning' ? 'text-[var(--color-warning-fg)]' : 'text-[var(--color-error-fg)]'
              }`}>
                {result.ok ? 'Ready to Upload' : result.status === 'warning' ? 'Needs Review' : 'Needs Retake'}
              </span>
            </div>
            ...
            {result.type && (
              <p className="mt-0.5 text-xs text-[var(--color-muted)] uppercase tracking-wider font-bold">
                {result.type} {result.width && result.height ? `${result.width}×${result.height}` : ''}
              </p>
            )}
          </div>
        </div>
      ...
      {result.issues.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {result.issues.map((issue) => (
            <span key={issue} className="badge bg-[var(--color-error-bg)] text-[var(--color-error-fg)] border-[var(--color-error-fg)]">
              {issue.replace(/_/g, ' ')}
            </span>
          ))}
        </div>
      ...
      {'warnings' in result && Array.isArray(result.warnings) && result.warnings.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {result.warnings.map((warning) => (
            <span key={warning} className="badge bg-[var(--color-warning-bg)] text-[var(--color-warning-fg)] border-[var(--color-warning-fg)]">
          ...
          {checks.map(([name, check]) => (
            <div key={name} className={`check-card ${getStatusColor(check.status)}`}>
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-bold capitalize">{name === 'file' ? 'Format' : name}</span>
          ...
          <div className="space-y-1">
            {result.pages.map((page) => (
              <div key={page.page}>
                ...
                >
                  <div className="flex items-center gap-2">
                    {expandedPages[page.page] ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                    <span>Page {page.page}</span>
                    {page.orientation && (
                      <span className="text-xs text-[var(--color-muted)] capitalize">{page.orientation}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-mono font-bold tabular-nums ${
                      page.ok ? 'text-[var(--color-success-fg)]' : page.status === 'warning' ? 'text-[var(--color-warning-fg)]' : 'text-[var(--color-error-fg)]'
                    }`}>
                      {page.score}/100
                    </span>
                  </div>
                  ...
                  <div className="mt-1 p-3 border-2 border-[var(--color-border)] rounded bg-[var(--color-surface)] space-y-2">
                    ...
                    {page.issues.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {page.issues.map((issue) => (
                          <span key={issue} className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border border-[var(--color-border)] bg-[var(--color-error-bg)] text-[var(--color-error-fg)]">
                            {issue.replace(/_/g, ' ')}
                          </span>
                        ))}
                      </div>
                    ...
                    {page.width && page.height && (
                      <p className="text-xs text-[var(--color-muted)]">
                        {page.width}×{page.height} {page.rotation ? `rotated ${page.rotation}°` : ''}
                      </p>
                    )}
                  </div>
                )}
        ...
        <div className="panel p-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs font-mono">
            ...
            <div className="p-2 rounded border border-[var(--color-border)] bg-[var(--color-surface)]">
              ...
              <p className="font-bold">{result.blurAnalysis.metrics.edgeAnalysis.avgEdgeWidthPerc.toFixed(2)}%</p>
            </div>
          </div>
        </div>
      )}
      ...
      {showCode && <CodePreview result={result} config={config} onClose={() => setShowCode(false)} />}
    </div>
  )
}
