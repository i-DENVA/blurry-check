...
function Section({
  ...
  return (
    <section id={id} className="panel p-5 sm:p-6 space-y-4">
      <h2 className="text-xl font-black tracking-tight border-b-2 border-[var(--color-border)] pb-3">
        {title}
      </h2>
      ...
      <table className="w-full text-xs sm:text-sm">
        <thead>
          <tr className="bg-[var(--color-surface)] border-b-2 border-[var(--color-border)]">
            <th className="text-left p-2 sm:p-3 font-extrabold uppercase tracking-wider text-[10px] sm:text-xs">
              Property
            </th>
            <th className="text-left p-2 sm:p-3 font-extrabold uppercase tracking-wider text-[10px] sm:text-xs">
              Type
            ...
            <th className="text-left p-2 sm:p-3 font-extrabold uppercase tracking-wider text-[10px] sm:text-xs hidden sm:table-cell">
              Description
            </th>
          </tr>
        </thead>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <Section id="installation" title="Installation">
        ...
        <div className="panel-inset p-4 text-sm space-y-2">
          ...
          <ul className="list-disc pl-4 space-y-1 text-[var(--color-muted)]">
            ...
            <li>
              React / Next.js usage must run on the client via{' '}
              <code className="font-mono font-bold bg-[var(--color-surface)] px-1 rounded">
                &apos;use client&apos;
              </code>
              .
            </li>
        ...
        <p className="text-sm text-[var(--color-muted)]">
          The package requires a browser environment with canvas support. For Node.js server-side
          use, install the optional{' '}
          <code className="font-mono font-bold bg-[var(--color-surface)] px-1 rounded">canvas</code>{' '}
          peer dependency.
        </p>
      ...
      <Section id="quick-start" title="Quick Start">
        ...
        <CodeBlock
          code={`import { validateUpload } from 'blurry-check'
  ...
  uploadToServer(file)
      ...
      <Section id="core-modules" title="Core Modules">
        <SubSection title="BlurryCheck">
          ...
          <CodeBlock
            code={`import { BlurryCheck } from 'blurry-check'
...
const validation = await checker.validateUpload(file, { preset: 'general' })
        ...
        <SubSection title="BlurDetector">
          ...
          <CodeBlock
            code={`import { BlurDetector } from 'blurry-check'
...
console.log(result.isBlurry, result.confidence, result.metrics)`}
        ...
        <SubSection title="PDFAnalyzer">
          <p className="text-sm text-[var(--color-muted)]">
            Full PDF quality analysis with multi-scale rendering, text sharpness, and document frame
            detection.
          </p>
          <CodeBlock
            code={`import { PDFAnalyzer } from 'blurry-check'
...
console.log(result.isQualityGood, result.isScanned, result.pagesAnalyzed)`}
        ...
        <SubSection title="Validators">
          ...
          <CodeBlock
            code={`import { validateImage, validateUpload } from 'blurry-check'
...
const uploadResult = await validateUpload(file, {
  ...
  minScore: 75,
      ...
      <Section id="configuration" title="Configuration">
        <p className="text-sm text-[var(--color-muted)]">
          All available configuration options for{' '}
          <code className="font-mono font-bold bg-[var(--color-surface)] px-1 rounded">
            BlurryCheck
          </code>{' '}
        ...
        <Table
          rows={[
            {
              prop: 'method',
              type: '"edge" | "laplacian" | "both"',
              default: '"both"',
              description: 'Detection method',
            },
            {
              prop: 'edgeWidthThreshold',
              type: 'number',
              default: '0.3',
              description: 'Lower = more sensitive to blur',
            },
            {
              prop: 'laplacianThreshold',
              type: 'number',
              default: '150',
              description: 'Higher = more sensitive to blur',
            },
            {
              prop: 'openCvUrl',
              type: 'string',
              default: 'OpenCV CDN',
              description: 'Custom OpenCV.js URL',
            },
            {
              prop: 'canvas',
              type: 'HTMLCanvasElement',
              default: 'auto-created',
              description: 'Custom canvas for processing',
            },
            {
              prop: 'debug',
              type: 'boolean',
            ...
            },
            {
              ...
              type: 'number',
              default: '70',
              description: 'Minimum 0–100 score threshold',
            },
            {
              prop: 'minWidth',
              type: 'number',
            ...
            },
            {
              ...
              type: 'ValidationMode',
              default: "'general'",
              description: 'Calibrated preset for common uploads',
            },
            {
              prop: 'strictness',
              type: '"low" | "medium" | "high"',
              default: "'medium'",
              description: 'Shifts all thresholds stricter or more permissive',
            },
            {
              prop: 'maxPages',
              type: 'number',
              default: 'no limit',
              description: 'Max pages to analyze in PDFs',
            },
            {
              prop: 'samplePages',
              type: '"first" | "all" | "smart" | number[]',
              default: '"all"',
              description: 'Page sampling strategy for large PDFs',
            },
            {
              prop: 'maxRenderScale',
              type: 'number',
              default: '2.0',
              description: 'Maximum PDF render scale cap',
            },
            {
              prop: 'timeoutMs',
              type: 'number',
      ...
      <Section id="modes" title="Validation Modes">
        <p className="text-sm text-[var(--color-muted)]">
          Modes apply calibrated defaults for common upload workflows. Use{' '}
          <code className="font-mono font-bold bg-[var(--color-surface)] px-1 rounded">
            strictness
          </code>{' '}
          to shift thresholds per mode. The deprecated{' '}
          <code className="font-mono font-bold bg-[var(--color-surface)] px-1 rounded">preset</code>{' '}
          option is mapped to the closest mode internally.
        </p>
        <Table
          rows={[
            {
              ...
              type: 'mode',
              default: '—',
              description: '600×600 min, 10MB, score 70+, method: both',
            },
            {
              prop: 'document',
              type: 'mode',
              default: '—',
              description: '1000×1000 min, 15MB, score 78+, method: edge, orientation check',
            },
            {
              prop: 'receipt',
              type: 'mode',
              default: '—',
              description: '800×800 min, 10MB, score 72+, method: edge',
            },
            {
              prop: 'invoice',
              type: 'mode',
            ...
            },
            {
              ...
              type: 'mode',
              default: '—',
              description: '900×600 min, 10MB, score 80+, method: edge, landscape',
            },
            {
              prop: 'passport',
              type: 'mode',
              default: '—',
              description: '900×600 min, 8MB, score 82+, method: edge, threshold 0.22',
            },
            {
              prop: 'profile-photo',
              type: 'mode',
              default: '—',
              description: '400×400 min, 8MB, score 75+, jpg/png/webp, method: both',
            },
            {
              prop: 'ocr',
              type: 'mode',
              default: '—',
              description: '1200×1200 min, 20MB, score 80+, method: edge, threshold 0.18',
            },
            {
              prop: 'ai-input',
              type: 'mode',
              default: '—',
              description: '800×800 min, 15MB, score 72+, method: edge, relaxed checks',
            },
          ]}
        />
      </Section>
      ...
      <Section id="image-checks" title="Image Checks">
        <p className="text-sm text-[var(--color-muted)]">
          Each image check returns its own{' '}
          <code className="font-mono font-bold bg-[var(--color-surface)] px-1 rounded">
            QualityCheckResult
          </code>{' '}
          with status, score, and a user-facing message.
        </p>
        <Table
          rows={[
            {
              ...
              default: '',
              description:
                'Sobel edge analysis or OpenCV Laplacian. Detects blurry and out-of-focus images.',
            },
            {
              prop: 'brightness',
              type: '',
              default: '',
              description:
                'Luminance mean across all pixels. Ideal range: 80–210. Flags under/overexposure.',
            },
            {
              prop: 'contrast',
              type: '',
              default: '',
              description:
                'Standard deviation of luminance. Flags washed-out or flat images below 28.',
            },
            {
              prop: 'resolution',
              type: '',
              default: '',
              description: 'Enforces min/max width and height. Prevents low-res uploads.',
            },
            {
              prop: 'fileSize',
              type: '',
              default: '',
              description: 'Checks file size against maxSizeMB or maxSizeBytes.',
            },
            {
              prop: 'format',
              type: '',
              default: '',
              description: 'Validates MIME type, extension, and binary magic bytes.',
            },
          ]}
        />
      </Section>
          ...
          rows={[
            {
              ...
              default: '',
              description:
                'Detects whether the PDF is scanned (image-based) or has extractable digital text.',
            },
            {
              prop: 'sharpness',
              type: '',
              default: '',
              description:
                'Per-page blur analysis using multi-scale rendering and text region sampling.',
            },
            {
              prop: 'textDensity',
              type: '',
              default: '',
              description:
                'Warns when a page has very little extractable text. Exempts certificate documents.',
            },
            {
              prop: 'orientation',
              type: '',
              default: '',
              description: 'Detects portrait vs landscape vs square. Flags unexpected rotations.',
            },
            {
              prop: 'corruptedPages',
              type: '',
              default: '',
              description: 'Tracks pages that could not be rendered or analyzed.',
            },
            {
              prop: 'mobileCapture',
              type: '',
              default: '',
              description:
                'Detects missing document edges and perspective distortion from angled camera shots.',
            },
            {
              prop: 'pageResolution',
              type: '',
              default: '',
              description: 'Minimum dimension check per rendered page.',
            },
            {
              prop: 'brightness',
              type: '',
              default: '',
              description: 'Per-page luminance analysis.',
            },
            {
              prop: 'contrast',
              type: '',
              default: '',
              description: 'Per-page luminance standard deviation.',
            },
          ]}
        />
      </Section>
      ...
      <Section id="examples" title="Real-World Examples">
        <SubSection title="Profile Photo Upload">
          <CodeBlock
            code={`import { validateImage } from 'blurry-check'
