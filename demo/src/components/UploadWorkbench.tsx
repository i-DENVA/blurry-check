...
export default function UploadWorkbench() {
    ...
    async (file: File) => {
      ...
      try {
        const checker = new BlurryCheck({
          ...
          edgeWidthThreshold: config.edgeWidthThreshold,
          laplacianThreshold: config.laplacianThreshold,
          debug: false,
        });
        const r = (await checker.validateUpload(file, {
          ...
          maxRenderScale: config.maxRenderScale,
          expectedOrientation:
            config.expectedOrientation === 'auto' ? undefined : (config.expectedOrientation as any),
  ...
  return (
    <section
      ...
      className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-20 border-t-[3px] border-[var(--color-border)]"
    >
      <div className="text-center mb-10">
        <h2 id="demo-heading" className="text-2xl sm:text-3xl font-black tracking-tight">
          Try It Yourself
        </h2>
        <p className="mt-3 text-[var(--color-muted)]">
          Upload an image or PDF to see real-time quality validation.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div
            className={`panel p-8 text-center cursor-pointer transition-colors ${dragOver ? 'bg-[var(--color-surface)]' : ''}`}
            onDrop={handleDrop}
            onDragOver={(e) => {
              e.preventDefault();
              setDragOver(true);
          ...
          >
            ...
            {isAnalyzing ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  <span className="btn-primary text-sm">Select a File</span>
                  <p className="text-xs text-[var(--color-muted)] mt-2">or drag and drop</p>
                </div>
        ...
        <div>
          {result ? (
            <ValidationResults result={result} config={config} />
          ) : (
