      ...
      {open && (
        <div className="mt-4 space-y-4">
          ...
          <Select
            ...
            onChange={(v) => onChange({ strictness: v as Strictness })}
            options={['low', 'medium', 'high']}
          />
          <Select
            label="Detection Method"
            value={config.method}
            onChange={(v) => onChange({ method: v as DetectionMethod })}
            options={['edge', 'laplacian', 'both']}
          />
          <Slider
            ...
            max={2}
            step={0.1}
            onChange={(v) => onChange({ edgeWidthThreshold: v })}
            unit=""
          ...
          <Slider
            ...
            max={300}
            step={10}
            onChange={(v) => onChange({ laplacianThreshold: v })}
            unit=""
            hint="Higher = more sensitive"
          />
          <Slider
            label="Minimum Score"
          ...
          />
          <Slider
            label="PDF Render Scale"
            value={config.maxRenderScale}
            min={1}
            max={3}
            step={0.5}
            onChange={(v) => onChange({ maxRenderScale: v })}
            unit="x"
            hint="Higher = more accurate but slower"
          />
          <Select
            label="Expected PDF Orientation"
            value={config.expectedOrientation}
            onChange={(v) => onChange({ expectedOrientation: v })}
